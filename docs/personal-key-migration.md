...
Gatepass doesn't manage your personal GPG key. It records the key's fingerprint (`humanKeyId` in `~/.config/gatepass/config.json`) and uses the key as a `pass` recipient, but generating it, moving it across machines, and trusting it is something you do with `gpg` directly — exactly the same way you would for signed git commits, encrypted email, or any other GPG-consuming tool.

This guide is the GPG-side companion to a gatepass migration. Do these steps first; *then* run `gatepass import` (or `gatepass setup`) on the new workstation.
...
- Setting up a brand-new personal identity — just run `gatepass setup` on the new machine and let it generate a new personal key.
- Setting up a bot host (`gatepass setup --bot-only`) — bot hosts have no personal key.
...
echo gatepass | gpg --clearsign --local-user "$FPR" >/dev/null   # signing works without an "untrusted" prompt
...
(This is the same gotcha `gatepass import-key --as-bot` works around automatically for the bot key. For a personal key, you do it yourself.)

## After your personal key is in place

The new machine now has the same personal identity as the source. From here:

- **Already have a `gatepass export` bundle for this machine?** Run `gatepass import bundle.gpg`. If the bundle includes the password store (`--include-store`), the restored `.gpg-id` will list your now-imported personal key as a recipient and `gatepass add` will work immediately. `gatepass doctor` should come up green.
- **Don't have a bundle?** Run `gatepass setup`. The wizard will list your imported personal key in the picker — choose it, and pass will be initialized with bot + personal as recipients.

The order matters: import the personal key **before** `gatepass import` if the bundle has a password store, so the recipient is already present when `pass` tries to round-trip the entries.
...
```bash
# on workstation 2, after running gatepass setup (which generated ws2's own personal key):
cd ~/.password-store
pass init "$(jq -r .botKeyId ~/.config/gatepass/config.json)" <ws1-personal-fpr> <ws2-personal-fpr>
```
...
## What gatepass does and doesn't do here

To be explicit about the line:

- Gatepass **records** your personal key's fingerprint and uses it as a `pass` recipient.
- Gatepass **doesn't move, generate, or back up** your personal key — that's `gpg`'s job (or yours).
- Gatepass **doesn't include** your personal key in `gatepass export` bundles. The bundle carries the bot key only. Move the personal key with this guide first, then run `gatepass import`.
- `gatepass setup` will offer to generate a personal key only if you have no GPG secret keys at all. It never overwrites or alters an existing personal key.
