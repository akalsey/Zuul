...
├── bot/             # default ZUUL_NAMESPACE — the entries zuul list shows
│   ├── metabase.gpg
│   └── ...
└── ...
```

Every `.gpg` file is encrypted to the bot key (and to your personal key, if you have one configured), so the contents are safe over any transport. What matters is that the **directory layout is preserved on the bot host** — `bot/metabase.gpg` must land at `~/.password-store/bot/metabase.gpg`, not at `~/.password-store/metabase.gpg`. If the namespace gets flattened, `zuul list` reports nothing even though the files are there.

## Don't write from two places at once

`pass insert` (which `zuul add` calls) is not designed for concurrent writes from multiple hosts. Pick one machine — usually the workstation — as the canonical writer, and have the bot host receive only. If the bot host ever runs `zuul add` interactively, sync the bot's store back to the workstation before you add anything else there.
...
4. Add `.stignore` if you want to exclude anything; for a vanilla zuul setup you don't need to.
...
`zuul list` should show every entry you added on the workstation. If it shows nothing but the `.gpg` files are present, you almost certainly flattened the namespace — see [troubleshooting.md](troubleshooting.md).
