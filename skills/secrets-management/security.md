...
Even though the bot user can read the encrypted files in `~/.password-store/personal/`, they're encrypted only to `my-key`. The bot's `poppy-key` is not a recipient, so GPG cannot decrypt them. This is enforced cryptographically, not by filesystem permissions.
...
**Plaintext credentials file** (e.g. `~/.openclaw/poppy-credentials.json`)
